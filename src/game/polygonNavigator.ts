...
import { PositionNavigator, GetNextPositionParameterObject, normalize, add, scale } from "./positionNavigator";
...
export class PolygonNavigator implements PositionNavigator {
    ...
    public constructor(polygon: readonly g.CommonOffset[], safeArea: g.CommonArea) {
    ...
    public containsPoint(point: g.CommonOffset): boolean {
        return this._safeArea.x <= point.x && this._safeArea.x + this._safeArea.width >= point.x && this._safeArea.y <= point.y && this._safeArea.y + this._safeArea.height >= point.y;
    }
    ...
    public getRandomPoint(rect: g.CommonRect): g.CommonOffset {
        return {
            x: this._safeArea.x + g.game.random.generate() * (this._safeArea.width - (rect.left + rect.right)) + rect.left,
            y: this._safeArea.y + g.game.random.generate() * (this._safeArea.height - (rect.top + rect.bottom)) + rect.top,
        };
    }
}
