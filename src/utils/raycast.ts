 ...
 */
export function raycastLines(lines: readonly Line[], ray: Ray, distance: number, hasDirection?: boolean): RaycastHit | null {
 ...
 * @param hasDirection 衝突判定を行う線分に向きを持たせます。向きを持たせた場合、レイが始点から見た線分に対して左側から来ている場合のみ衝突判定を行います。
 */
export function raycastLine(line: Line, ray: Ray, distance: number, hasDirection?: boolean): RaycastHit | null {
...
export function rectRaycastLines(lines: readonly Line[], rect: Rect, ray: Ray, distance: number, hasDirection?: boolean): RaycastHit | null {
    let closestHit: RaycastHit | null = null;
...
export function rectRaycastLine(line: Line, rect: Rect, ray: Ray, maxDistance: number, hasDirection?: boolean): RaycastHit | null {
    let closestHit: RaycastHit | null = null;

    // 左上
    let offsetRay = {position: {x:ray.position.x - rect.left, y:ray.position.y - rect.top}, direction: ray.direction};
    let hit = raycastLine(line, offsetRay, maxDistance, hasDirection);
    if(hit)
    {
        closestHit = {
            position: {x:hit.position.x + rect.left, y:hit.position.y + rect.top}, // オフセットを打ち消す
    ...
    // 右上
    offsetRay = {position: {x:ray.position.x + rect.right, y:ray.position.y - rect.top}, direction: ray.direction};
    hit = raycastLine(line, offsetRay, maxDistance, hasDirection);
    if(hit && (!closestHit || hit.distance < closestHit.distance)){
        closestHit = {
            position: {x:hit.position.x - rect.right, y:hit.position.y + rect.top}, // オフセットを打ち消す
    ...
    // 左下
    offsetRay = {position: {x:ray.position.x - rect.left, y:ray.position.y + rect.bottom}, direction: ray.direction};
    hit = raycastLine(line, offsetRay, maxDistance, hasDirection);
    if(hit && (!closestHit || hit.distance < closestHit.distance)){
        closestHit = {
            position: {x:hit.position.x + rect.left, y:hit.position.y - rect.bottom}, // オフセットを打ち消す
            distance: hit.distance,
            normal: hit.normal
        };
    }
    ...
    // 右下
    offsetRay = {position: {x:ray.position.x + rect.right, y:ray.position.y + rect.bottom}, direction: ray.direction};
    hit = raycastLine(line, offsetRay, maxDistance, hasDirection);
    if(hit && (!closestHit || hit.distance < closestHit.distance)){
        closestHit = {
            position: {x:hit.position.x - rect.right, y:hit.position.y - rect.bottom}, // オフセットを打ち消す
            distance: hit.distance,
            normal: hit.normal
        };
    }
